...
function uploadStory() {
  var story = makeFakeStory()
  db.postStory(story, function(err, cursor) {
    if (err) {
      return "There was an error uploading fake story"
    } else {
      console.log('Story upload success')
      uploadImage(story.id)
      return db.disconnect();
    }
  })
}

function makeFakeStory() {
  var id = guid()
  var fakeStoryObject = {
    id: id,
    name: chance.name(),
    ...
    img: "https://s3-us-west-2.amazonaws.com/worldinme-full/" + id + ".jpg",
    date: Date.now()
  }

  return fakeStoryObject
}
...
function uploadImage(id) {
  ...
  fs.readFile(imgUrl, function(err, data) {
    ...
    processImage(data, id, function(err, res) {
      if (err) {
        return console.log('error huh?', err)
      } else {
